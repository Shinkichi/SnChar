...
class ZombieFleshpound extends SnMod.ZombieFleshpoundBase
    abstract;
...
function TakeDamage( int Damage, Pawn InstigatedBy, Vector Hitlocation, Vector Momentum, class<DamageType> damageType, optional int HitIndex)
{
        ...
        && DamageType != class 'DamTypeSeekerSixRocket' )
	{
		...
		if( bIsHeadShot && class<KFWeaponDamageType>(damageType)!=none &&
		    class<KFWeaponDamageType>(damageType).default.HeadShotDamageMult >= 1.25 )
		{
			Damage *= 0.75;
		}
		else
...
defaultproperties
{
    ...
    EventClasses(0)="SnChar.ZombieFleshpound_STANDARD"
